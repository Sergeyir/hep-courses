...
I recommend using [ROOT::Math::IntegratorMultiDim](https://root.cern.ch/doc/master/classROOT_1_1Math_1_1IntegratorMultiDim.html) with VEGAS algorithm [3](#sources) for C++ and [vegas](https://vegas.readthedocs.io/en/latest/index.html) for python for multidimensional integration. You can find code examples for multidimensional integration in hep-courses/examples directory ([here](../examples/NDimIntegration.cpp) for C++, and [here](../examples/ndim_integration.py) for python).

### Final calculations

$d \sigma / dp_T$ can be obtained by estimating $\Delta y_1$ as bin rapidity coverage (or as an approximation of different $p_T$ bin rapidity coverages) for each $p_T$ bin.
...
3. [Lepage, G. Peter. "Adaptive multidimensional integration: vegas enhanced". Journal of Computational Physics 439 (2021) 110386](https://arxiv.org/abs/2009.05112)
